...
export const BUILDINGS = [
  {
    ...
    base: { gold: 250, wood: 200, stone: 120 }, mul: 1.62,
    stats: (l) => ({
      'Level cap':  { v: l + 1, s: '' },
  ...
  },
  {
    ...
    base: { gold: 60, wood: 40 }, mul: 1.46,
    stats: (l) => ({ 'Food / sec': { v: +(0.55 * Math.pow(1.3, l - 1)).toFixed(2), s: '' } }),
  },
  {
    ...
    base: { gold: 50, wood: 25 }, mul: 1.44,
    stats: (l) => ({ 'Wood / sec': { v: +(0.7 * Math.pow(1.3, l - 1)).toFixed(2), s: '' } }),
  },
  {
    ...
    base: { gold: 90, wood: 60 }, mul: 1.48,
    stats: (l) => ({ 'Stone / sec': { v: +(0.4 * Math.pow(1.3, l - 1)).toFixed(2), s: '' } }),
  },
  {
    ...
    base: { gold: 130, wood: 70, stone: 40 }, mul: 1.5,
    stats: (l) => ({ 'Gold / sec': { v: +(0.9 * Math.pow(1.3, l - 1)).toFixed(2), s: '' } }),
  },
  {
    ...
    base: { gold: 190, wood: 130, stone: 60 }, mul: 1.52,
    stats: (l) => ({
      'Population': { v: l * 4, s: '+' },
      'Gold bonus': { v: l * 6, s: '%' },
    }),
  },
  {
    ...
    base: { gold: 150, wood: 100, stone: 50 }, mul: 1.5,
    unit: 'militia',
    stats: (l) => ({
      'Militia power': { v: Math.round(100 * Math.pow(1.3, l - 1)), s: '%' },
  ...
  },
  {
    ...
    base: { gold: 185, wood: 145, stone: 40 }, mul: 1.5,
    unit: 'archer',
    stats: (l) => ({
      'Archer power': { v: Math.round(100 * Math.pow(1.3, l - 1)), s: '%' },
  ...
  },
  {
    ...
    base: { gold: 420, wood: 210, stone: 240 }, mul: 1.55,
    unit: 'mage', req: { townhall: 6 },
    stats: (l) => ({
      'Mage power':  { v: Math.round(100 * Math.pow(1.3, l - 1)), s: '%' },
  ...
  },
  {
    ...
    base: { gold: 230, wood: 120, stone: 150 }, mul: 1.52,
    stats: (l) => ({
      'Army attack': { v: l * 9, s: '%' },
      'Army health': { v: l * 7, s: '%' },
    }),
  },
  {
    ...
    base: { gold: 170, wood: 110, stone: 130 }, mul: 1.5,
    stats: (l) => ({
...
export const TROOPS = {
  militia: { name: 'Militia', from: 'barracks', pop: 1, food: 0.05,
    hp: P(34, 1.3), dmg: P(5.5, 1.3), range: 9,  atk: 1.0,  spd: 15, w: 6, h: 9 },
  knight:  { name: 'Knight',  from: 'barracks', pop: 2, food: 0.09, minLvl: 8, every: 3,
    hp: P(120, 1.3), dmg: P(15, 1.3), range: 10, atk: 1.15, spd: 12, w: 7, h: 10 },
  archer:  { name: 'Archer',  from: 'archery',  pop: 1, food: 0.05,
    hp: P(20, 1.3), dmg: P(7, 1.3),  range: 44, atk: 0.95, spd: 14, w: 6, h: 9, ranged: true },
  mage:    { name: 'Mage',    from: 'magetower', pop: 2, food: 0.11,
    hp: P(26, 1.3), dmg: P(15, 1.3), range: 50, atk: 0.5,  spd: 12, w: 6, h: 10, ranged: true, splash: true },
};
...
// ---------------------------------------------------------------------------
export const FOES = {
  goblin: { name: 'Goblin',   hp: 26,  dmg: 4,  range: 9,  atk: 1.0, spd: 17, w: 6, h: 8,  bounty: 3 },
  wolf:   { name: 'Dire Wolf',hp: 18,  dmg: 6,  range: 8,  atk: 0.8, spd: 27, w: 9, h: 7,  bounty: 4 },
  orc:    { name: 'Orc',      hp: 80,  dmg: 10, range: 10, atk: 1.2, spd: 12, w: 7, h: 10, bounty: 8 },
  shaman: { name: 'Shaman',   hp: 38,  dmg: 8,  range: 40, atk: 1.3, spd: 12, w: 6, h: 9,  bounty: 11, ranged: true },
  ogre:   { name: 'Ogre',     hp: 420, dmg: 26, range: 12, atk: 1.6, spd: 8,  w: 12,h: 15, bounty: 60, boss: true },
};

// Quadratic-ish so late waves bite without the exponential blow-up that makes
// idle games unplayable around wave 40.
export const waveHp    = (w) => 1 + 0.16 * (w - 1) + 0.008 * (w - 1) * (w - 1);
export const waveDmg   = (w) => 1 + 0.12 * (w - 1) + 0.004 * (w - 1) * (w - 1);
export const waveCount = (w) => Math.min(46, 4 + Math.floor(w * 0.75));
export const waveGold  = (w) => Math.round(45 * Math.pow(1.13, w - 1));
